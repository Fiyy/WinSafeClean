...
当前 Windows provider 通过骨架逐步填充真实读取逻辑。服务 `ImagePath` 读取已在 ADR 0011 中实现和记录，计划任务 action 读取已在 ADR 0012 中实现和记录，启动项注册表读取已在 ADR 0013 中实现和记录，其他 provider 仍需通过 TDD 单独实现。
...
- 当前会发现服务 `ImagePath`、计划任务 Exec action 和注册表启动项对目标文件的直接引用。

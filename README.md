...
目录扫描当前只读取直接子项；目录项大小暂记为 `0 B`，不会递归计算目录大小。

当前报告 schema 为 `1.2`。每个报告项包含路径、`itemKind`、大小、可空 `lastWriteTimeUtc` 和风险判断；时间戳读取失败时只输出 `null` 或 Markdown `-`，不会中断扫描。`--privacy redacted` 会替换路径 token 并抑制修改时间，适合分享报告时降低直接泄露风险。

...
- `WinSafeClean.Ui`：WPF UI 脚手架，可读取 scan report、preflight checklist 和 cleanup plan JSON，并展示大小、最大条目、Top 目录、空间用途提示、风险、动作、证据、原因、检查项、处置建议、隔离预览、空状态和状态提示；Read-Only Ops 可通过 Quick Start、文件/文件夹选择器构建并运行 `scan`、`plan` 和 `preflight` 只读命令，把 JSON 输出自动加载回界面，并可从选中的 cleanup plan 候选项准备 preflight metadata 输入
